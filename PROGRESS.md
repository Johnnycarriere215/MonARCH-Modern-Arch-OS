...
**Phase:** 2 complete — Phase 3 next, after the desktop has actually booted and had a week of use
**Next task:** the VM (or USB) test. **Not T9.** Building code on an unbooted desktop is why there is a review-and-docs backlog instead of a working system.
**Blocked on:** a first boot. **Phase 2 is complete and none of T1–T8 has ever been run on Arch** — no Hyprland has parsed a line MonARCH generates. Everything past here needs that: T7's rollback criterion, T2's "boots to a desktop and opens a terminal", the whole "live in it for a week before the GUI" gate.

**Do not push the human to test unprompted.** As of 2026-08-08 they were offered the VM build and chose *not yet*. The install machinery all exists (`/dev/kvm` works on their Mint box, qemu + virt-manager installed); the human drives when ready. The three routes: `docs/07-VM-TESTING.md` (VM), `docs/08-USB-INSTALL.md` (external drive, internal untouched), `docs/10-DUAL-BOOT.md` (beside Mint). The fastest loop is them starting it and pasting back the failing stage + `monarch doctor`. **The AI cannot do the visual/interactive acceptance itself** — no framebuffer, no keys at the greeter — only headless smoke-checks.

**If asked to build more anyway:** everything sensible is built. T9 (GUI) is deliberately gated on a week of real use, and designing it blind is guesswork. Prefer fixing what a boot turns up over adding surface.
...
2026-08-08 | editor + branding | done | Locked decision changed at the human's request: MonARCH Code (Tauri+Monaco, repo Johnnycarriere215/MonARCH-Code) replaces VS Code as the default editor — Super+C, `monarch install monarch-code` builds from source, VS Code moved to optional.packages. New install/45-branding.sh brands the running system from brand/monarch.ascii: /etc/os-release (ID=monarch, ID_LIKE=arch — load-bearing), /etc/issue on the TTYs, tuigreet greeting (set in stage 40, single-quoted — a sed'd double-quote there was a real bug that would have left no login screen), fastfetch. Bare `monarch` and `monarch version --logo` print the wordmark. Migration 1786226624.
2026-08-08 | install docs | done | Written for the human trying to actually run this: docs/09-ARCH-INSTALL.md (archinstall step by step), 07-VM-TESTING.md rewritten to link it, 08-USB-INSTALL.md (daily-drive from an external drive, internal disk untouched), 10-DUAL-BOOT.md (beside Mint — verified MonARCH's installer never touches the bootloader, so risk is contained to archinstall). README rewritten with the real logo + an honest 'never been booted' banner. docs/MonARCH-install-card.pdf — two-page printable. This machine is the EliteBook: 931G NVMe, ext4 Mint, 759G free.
2026-08-08 | session upkeep | VM deferred | Explained what building+testing the VM entails; human chose 'not yet'. Nothing built or downloaded. Updated Current state so the next session does not re-push the test. Repo clean and pushed at this point.
